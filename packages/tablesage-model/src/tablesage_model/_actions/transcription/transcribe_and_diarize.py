...
async def transcribe_and_diarize(
    ...
    for word in transcribed_words:
        ...
        if word.speaker != current_speaker:
            utterances.append(Utterance.from_words(current_words))
            current_words = []
            current_speaker = word.speaker
        current_words.append(Word(text=word.text, start=word.start, end=word.end, speaker=word.speaker))
    if current_words:
        utterances.append(Utterance.from_words(current_words))
    ...
    return Discourse(utterances=tuple(utterances))
