...
async def test_no_candidates_returns_transcript_unchanged(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    result = await remove_backchannels(transcript, max_words=3, llm_model="anthropic/claude-haiku-4-5")

    assert result == transcript


@pytest.mark.anyio
async def test_removes_candidate_judged_not_an_answer(monkeypatch: pytest.MonkeyPatch) -> None:
    transcript = _transcript_with_candidate()

    async def _stub_call_llm_with_prompt(*args: object, **kwargs: object) -> str:
        return BackchannelJudgments(scratchpad="", judgments=[BackchannelJudgment(candidate_id=1, is_answer=False)]).model_dump_json()
...
@pytest.mark.anyio
async def test_keeps_candidate_judged_an_answer(monkeypatch: pytest.MonkeyPatch) -> None:
    transcript = _transcript_with_candidate()

    async def _stub_call_llm_with_prompt(*args: object, **kwargs: object) -> str:
        return BackchannelJudgments(scratchpad="", judgments=[BackchannelJudgment(candidate_id=1, is_answer=True)]).model_dump_json()
