...
class Stage(Enum):
    ...
    TRANSCRIBING and PUNCTUATING are each a single opaque call -- `on_progress`
    fires with `total=0` on entry (indeterminate: show the stage label, not a
    moving bar) and `(1, 1)` on completion. IDENTIFYING_SPEAKERS is itemized
    per utterance and reports real `(completed, total)` throughout.
    """
...
def transcribe_audio(
    ...
    timestamped, speaker-labeled script for humans) there, only once every
    stage has succeeded -- a failure partway through leaves no artifacts
    behind, matching `import_audio`'s all-or-nothing contract. `centroids`
    should be scoped to the session's attendees; its size is passed through to
    diarization as the expected speaker count, since `identify_speakers`
    already assumes exactly that correspondence.
    """
