...
The three Brandonsford inputs are snapshots of version-1 Scene Breakdowns, rendered through the production Recap Summary user template. Production now generates each breakdown jointly with its Ledger from the routed Role Transcript; see the [artifact specification](../../.work-items/scene-breakdown-artifact-contract/specification.md).
