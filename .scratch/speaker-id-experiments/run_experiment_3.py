"""Experiment #3 driver: run the `wespeaker-resnet34` candidate (WeSpeaker ResNet34-LM embedder,
`benchmarks/speaker_id/embedders.py`'s `WeSpeakerResNet34Embedder`) against the frozen benchmark
...
local-model-dir override used there. `wespeaker-resnet34` itself needs no such override --
its checkpoint is pulled from huggingface.co, which this sandbox can reach.

Usage (from the repo root, inside the venv):

    uv run python .scratch/speaker-id-experiments/run_experiment_3.py
"""
...
def main() -> None:
    ...
    wespeaker_resnet34 = WeSpeakerResNet34Embedder()
