...
- `.documentation/` holds internal reference material, separate from tracked work: maintained implementation guides at its root, retained feature designs in `designs/`, developer references in `developer/`, research in `research/`, untracked conceptual references in `ideas/`, and repository maintenance history in `maintenance/`. Use [.documentation/INDEX.md](../.documentation/INDEX.md) to navigate it. Canonical artifact specifications are retained in their completed contract work items; developer/corpus READMEs stay beside their tools and data.
- Do not use `.scratch/`, `.documents/`, `.ideas/`, or `.research/` as competing document or work-item stores. Existing experiment scripts, measurement data, logs and art assets may remain at their original paths. Link retained reference documents from relevant work items instead of copying them. A historical design does not become active work merely by being retained or moved.
- Do not publish issues to an external tracker unless the user explicitly requests it. Historical triage labels describe past decisions, not current work-item stages; preserve their meaning in prose rather than inventing a stage transition.
